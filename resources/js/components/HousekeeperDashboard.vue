                    ...
                    <v-card-text class="pa-4 flex-grow-1 d-flex flex-column justify-space-between">
                      <div>
                        ...
                        <div v-else class="text-center mb-3">
                          ...
                          <v-alert v-if="currentAssignment !== 'N/A' && canClockIn" type="warning" variant="tonal" density="compact" class="mb-3">
                            <div class="text-caption" style="color: #000000;">
                          ...
                          <v-alert v-else-if="currentAssignment !== 'N/A' && !canClockIn" type="info" variant="tonal" density="compact" class="mb-3">
                            <div class="text-caption" style="color: #000000;">
...
const handleRequestPayout = async () => {
  ...
  // await fetch(`/api/housekeeper/${caregiverId.value}/request-payout`, { method: 'POST' });
};
...
const checkApplicationStatus = async () => {
  try {
    const response = await fetch('/api/housekeeper/application-status');
...
const loadAvailableClients = async () => {
  try {
    const response = await fetch('/api/housekeeper/available-clients');
...
const loadScheduleEvents = async () => {
  try {
    ...
    const response = await fetch(`/api/housekeeper/schedule-events?caregiver_id=${caregiverId.value}&month=${month}&year=${year}`);
    const data = await response.json();
...
const loadEarningsReportData = async () => {
  try {
    const response = await fetch(`/api/housekeeper/${caregiverId.value}/earnings-report?period=${encodeURIComponent(earningsReportPeriod.value)}`);
    const data = await response.json();
...
const exportEarningsReport = async () => {
  ...
  try {
    const response = await fetch('/api/housekeeper/earnings-report-pdf', {
      method: 'POST',
      headers: {
...
const currentAssignment = ref('N/A');
...
const canClockIn = computed(() => {
  return currentAssignment.value !== 'N/A' && isBookingActive.value && hasStartTimePassed.value && !isTimedIn.value;
});

const bookingStatusMessage = computed(() => {
  if (currentAssignment.value === 'N/A') {
...
const handleTimeIn = async () => {
  try {
    if (!caregiverId.value) {
      alert('Housekeeper ID not found. Please refresh the page.');
      return;
    }
    ...
    const response = await fetch('/api/time-tracking/clock-in', {
      ...
      body: JSON.stringify({
        housekeeper_id: caregiverId.value,  // Use housekeeper_id for housekeepers
...
const stats = ref([
  { title: 'Current Assignment', value: 'N/A', icon: 'mdi-account', color: 'grey', change: 'Status: No Contract', changeColor: 'text-grey', changeIcon: 'mdi-close-circle' },
...
const loadProfile = async () => {
  try {
    ...
    if (data.user) {
      ...
      // Fallback for demo purposes - use Demo Housekeeper ID
...
const loadCaregiverStats = async () => {
  try {
    ...
    const response = await fetch(`/api/housekeeper/${caregiverId.value}/stats`);
    const data = await response.json();
    ...
    if (!hasActiveClient) {
      try {
        const assignmentResponse = await fetch(`/api/housekeeper/${caregiverId.value}/assignments`);
...
if (hasActiveClient) {
    ...
    try {
      const earningsResponse = await fetch(`/api/housekeeper/${caregiverId.value}/earnings-report?period=This Week`);
      ...
      const prevWeekResponse = await fetch(`/api/housekeeper/${caregiverId.value}/earnings-report?period=Last Week`);
...
const loadCurrentSession = async () => {
  try {
    ...
    const response = await fetch(`/api/time-tracking/housekeeper/current-session/${caregiverId.value}`);
    const data = await response.json();
...
const connectBankAccount = async () => {
  try {
    ...
    // Navigate to housekeeper-specific bank connection page
    window.location.href = '/connect-bank-account-housekeeper';
...
const loadPaymentData = async () => {
  try {
    const response = await fetch('/api/housekeeper/payment-data');
    const data = await response.json();
...
const loadPastBookings = async () => {
  try {
    const response = await fetch('/api/housekeeper/past-bookings');
    const data = await response.json();
